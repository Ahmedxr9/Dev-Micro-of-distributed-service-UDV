...
Log.Logger = new LoggerConfiguration()
    ...
    .WriteTo.Elasticsearch(new ElasticsearchSinkOptions(new Uri(builder.Configuration["Elasticsearch:Uri"] ?? "http://localhost:9200"))
    {
        ...
        IndexFormat = "sms-service-{0:yyyy.MM.dd}"
    })
    .CreateLogger();
...
// SMS Service
builder.Services.AddScoped<ISMSService, SMSService.Services.SMSService>();
...
// Prometheus Metrics
builder.Services.AddSingleton<IMetricServer>(sp => new MetricServer(port: 9092));
